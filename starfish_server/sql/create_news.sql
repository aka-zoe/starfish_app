...
(
    ...
    `dianZan`       int comment '点赞',
    `pingLun`       int comment '评论数',
    `imageUrl`      varchar(255) comment '图片地址',
    `contentUrl`    varchar(255) comment '内容链接',
    `createTime`    int comment '创建时间 时间戳',
    `operationTime` int comment '修改时间 时间戳',
    `createBy`      int comment '创建人用户id',
    `editBy`        int comment '修改人用户id',
